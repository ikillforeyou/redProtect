...
public class RedefineRegionBuilder extends RegionBuilder {
	public RedefineRegionBuilder(Player p, Region old, Location l1, Location l2) {
		...
		for (int xl = minX; xl <= maxX; xl++) {
			if (RedProtect.rm.regionExists(xl, minZ, w)
					|| RedProtect.rm.regionExists(xl, maxZ, w)) {
				p.sendMessage(RED + "You're overlapping another region.");
				return;
			}
		}
		for (int zl = minZ; zl <= maxZ; zl++) {
			if (RedProtect.rm.regionExists(minX, zl, w)
					|| RedProtect.rm.regionExists(maxX, zl, w)) {
				setError(p, "You're overlapping another region.");
				return;
			}
		}
		Region r = new Region(old.getName(), old.getOwners(),
				new int[] { l1.getBlockX(), l1.getBlockX(), l2.getBlockX(),
						l2.getBlockX() }, new int[] { l1.getBlockZ(),
