...
public class DefineRegionBuilder extends RegionBuilder {
	public DefineRegionBuilder(Player p, Location loc1, Location loc2,
			String regionName, String creator) {
		...
		if ((loc1 == null) || (loc2 == null)) {
			p.sendMessage(RED
					+ "One or both of your selection positions aren't set!");
			return;
		}
		if (RedProtect.rm.getRegion(regionName, p.getWorld()) != null) {
			p.sendMessage(RED
		...
		for (int xl = minX; xl <= maxX; xl++) {
			if (RedProtect.rm.regionExists(xl, minZ, p.getWorld())
					|| RedProtect.rm.regionExists(xl, maxZ, p.getWorld())) {
				setError(p, RED + "You're overlapping another region.");
				return;
			}
		}
		for (int zl = minZ; zl <= maxZ; zl++) {
			if (RedProtect.rm.regionExists(minX, zl, p.getWorld())
					|| RedProtect.rm.regionExists(maxX, zl, p.getWorld())) {
				setError(p, RED + "You're overlapping another region.");
				return;
			}
		}
		...
		Region r = new Region(regionName, owners, new int[] { loc1.getBlockX(),
				loc1.getBlockX(), loc2.getBlockX(), loc2.getBlockX() },
				new int[] { loc1.getBlockZ(), loc1.getBlockZ(),
