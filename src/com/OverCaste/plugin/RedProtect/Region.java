...
@Deprecated
// This is the old region class, still included fot it to be able to be
// converted.
public class Region implements Serializable {
	...
	protected boolean[] f = { Flags.pvp, Flags.chest, Flags.lever,
			Flags.button, Flags.door, Flags.mobs, Flags.passive }; // flags, see
																	// Flags.java
	...
	public int getCenterZ() {
		return Double.valueOf(((double) (minMbrZ + maxMbrZ) / (double) 2))
				.intValue();
	}
	...
	@Deprecated
	public Region(int[] x, int[] z, String name, LinkedList<String> owners,
			List<String> members, String creator, int maxMbrX, int minMbrX,
			int maxMbrZ, int minMbrZ, boolean[] flags) {
		this.x = x;
		this.z = z;
	...
	public boolean intersects(int bx, int bz) {
		...
		for (i = 0, j = x.length - 1; i < x.length; j = i++) {
			if ((((z[i] <= bz) && (bz < z[j])) || ((z[j] <= bz) && (bz < z[i])))
	...
	public String getFlagInfo() {
		return (AQUA + "Player vs Player: " + GOLD + f[0] + AQUA
				+ ", Chest opening: " + GOLD + f[1] + AQUA
				+ ", Lever flipping: " + GOLD + f[2] + AQUA
				+ ", Button pushing: " + GOLD + f[3] + AQUA
				+ ", Door toggling: " + GOLD + f[4] + AQUA
				+ ", Monster spawning: " + GOLD + f[5] + AQUA
				+ ", Passive entity hurting: " + GOLD + f[6]);
	}
