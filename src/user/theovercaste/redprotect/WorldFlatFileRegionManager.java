...
public class WorldFlatFileRegionManager implements WorldRegionManager {
	...
	public void add(Region r) {
		...
		for (int xl = cMinX; xl <= cMaxX; xl++) {
			for (int zl = cMinZ; zl <= cMaxZ; zl++) {
				LargeChunkObject lco = regions
						.get(Location2I.getXZLong(xl, zl));
				if (lco == null) {
	...
	public boolean canBuild(Player p, Block b) {
		int bx = b.getX();
		int bz = b.getZ();
		LargeChunkObject lco = regions.get(LargeChunkObject.getBlockLCOLong(bx,
				bz));
		if (lco == null) {
			return true;
		}
	...
	public boolean regionExists(int x, int z) {
		LargeChunkObject lco = regions.get(LargeChunkObject.getBlockLCOLong(x,
				z));
	...
	public void save() {
		try {
			RedProtect.logger.debug("RegionManager.Save(): File type is "
					+ RedProtect.fileType.toString());
			String world = getWorld().getName();
			File datf = new File(RedProtect.pathData, "data_" + world
					+ ".regions");
			if (!datf.exists()) {
				datf.createNewFile();
			}
			HashMap<Long, Set<String>> lcos = new HashMap<Long, Set<String>>(
					regions.size());
			for (Entry<Long, LargeChunkObject> e : regions.entrySet()) {
				if (e == null || e.getValue() == null
						|| e.getValue().regions == null) {
					continue;
				}
			...
			case ymlgz:
				backupRegions(datf);
				Yaml ymlgz = new Yaml();
				ymlgz.dump(regions, new OutputStreamWriter(
						new GZIPOutputStream(new FileOutputStream(datf))));
				break;
			case oos:
				backupRegions(datf);
			...
			case oosgz:
				backupRegions(datf);
				ObjectOutputStream oosgz = new ObjectOutputStream(
						new GZIPOutputStream(new FileOutputStream(datf)));
	...
	private void backupRegions(File datf) {
		if (!RedProtect.backup)
			return;
		File dataBackup = new File(RedProtect.pathData, "data_"
				+ getWorld().getName() + ".regions.backup");
		dataBackup.delete();
				...
				if (other != null) {
					if (r.inBoundingRect(other.getCenterX(), other.getCenterZ())) {
						if (r.intersects(other.getCenterX(), other.getCenterZ())) {
							System.out
									.println("Intersecting!: " + other.info());
							return true;
						}
					}
				}
	...
	private void load(String path) {
		...
		if (!((f = new File(path)).exists())) {
			try {
				f.createNewFile();
				RedProtect.logger.info("Created new world region file: ("
						+ path + ").");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		try {
			if (!RPUtil.isFileEmpty(path)) {
				...
				case yml:
					RedProtect.logger
							.debug("RegionManager.load() File type: yml");
					Yaml yml = new Yaml();
					Object oyml;
				...
				case ymlgz:
					RedProtect.logger
							.debug("RegionManager.load() File type: ymlgz");
					Yaml ymlgz = new Yaml();
					Object oymlgz;
					if ((oymlgz = ymlgz.load(new GZIPInputStream(
							new FileInputStream(path)))) instanceof HashMap<?, ?>) {
				...
				case oos:
					RedProtect.logger
							.debug("RegionManager.load() File type: oos");
					if (ois == null) {
					...
					for (Entry<Long, Set<String>> ss : lcos.entrySet()) {
						regions.put(ss.getKey(), new LargeChunkObject(
								newRegions, ss.getValue()));
					}
				default:
					break;
				}
			} else {
				if (RedProtect.backup && backupExists()
						&& !path.equalsIgnoreCase(datbackf)) {
					load(datbackf);
					RedProtect.logger
							.info("Main data file is blank, Reading from backup.");
					return;
				}
			...
			if (RedProtect.backup && backupExists()
				...
				load(datbackf);
				RedProtect.logger
						.info("The data file is corrupt. Loading from backup.");
				return;
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public Set<Region> getRegionsNear(Player player, int area) {
		...
		for (int xl = cminX; xl <= cmaxX; xl++) {
			for (int zl = cminZ; zl <= cmaxZ; zl++) {
				...
				while (i.hasNext()) {
					Region poly = i.next();
					if (((Math.abs(poly.getCenterX() - px) <= area) && (Math
							.abs(poly.getCenterZ() - pz) <= area))) {
						ret.add(poly);
					}
				}
			}
